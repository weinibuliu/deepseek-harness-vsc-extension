 ...
 * block — so earlier blocks are final. This parser therefore freezes all but
 * the trailing {@link UNSTABLE_TAIL_BLOCKS} blocks and re-parses only the
 * source tail behind them.
 *
 ...
 * node falls back to a negative list-index key — unique within one update's
...
export class IncrementalMarkdownParser {
   ...
   * Idempotent for identical input, so callers may invoke it from render
   * paths that re-execute.
